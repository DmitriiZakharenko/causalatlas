---
name: agent09_contradiction_gap_detection
...
direction is ever resolved by this agent) and does not generate hypotheses (Agent 11).

## Inputs
- `data/graphs/<disease>/knowledge_graph.json` (Agent 6 output) — full node/edge set with
  PMIDs, species, and (where available) the source sentence from Agent 5.
- `data/graphs/<disease>/loops.json` (Agent 7 output) — for gap-scanning partial loops.
...
- A C-classified (conflicting literature) item from Agent 10 routes back here, not to
